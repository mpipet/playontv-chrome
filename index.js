...
var PlayOnTv = function () {
  function PlayOnTv(configuration) {
    _classCallCheck(this, PlayOnTv);
  ...
  _createClass(PlayOnTv, [{
    key: 'url',
    value: function url(endpoint) {
      return 'http://' + this.host + ':' + (this.port + this.endpoints[endpoint]);
    }
  }, {
    key: 'handlePlayOnTVClick',
    value: function handlePlayOnTVClick(info) {
      var _this = this;

      var url = info.linkUrl;
      this.getTorrentBinary(url, function (err, res) {
        // @TODO use file stream directly instead of b64 encoded Blob if possible
        var blob = new Blob([res.xhr.response], { type: 'application/x-bittorrent' });
        _this.blobToBase64(blob, function (b64Torrent) {
          return _this.loadAndPlayTorrent(b64Torrent);
        });
      });
    }
    ...
    key: 'getTorrentBinary',
    value: function getTorrentBinary(url, callback) {
    ...
    key: 'loadAndPlayTorrent',
    value: function loadAndPlayTorrent(b64Torrent) {
      var req = _superagent2.default.post(this.url('load'));
...
var playOnTv = new PlayOnTv(_config2.default);

chrome.contextMenus.onClicked.addListener(function (info, tab) {
  return playOnTv.handlePlayOnTVClick(info, tab);
});
